...
connect_network() {
    ...
    if [ "$BACKEND" = "nmcli" ]; then
        ...
        security=$(nmcli -f SSID,SECURITY dev wifi list 2>/dev/null | awk -v s="$network_name" '$1==s {$1=""; print}' | sed 's/^ *//')

        if echo "$security" | grep -q "^--\|^$"; then
